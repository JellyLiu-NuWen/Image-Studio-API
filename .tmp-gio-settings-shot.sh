...
xdotool mousemove --sync 1405 24 click 1
sleep 2

xwininfo -display :99 -root -tree > /src/.tmp-gio-settings-xwininfo.txt || true
window_id=$(awk '/"Image Studio Gio"/ {print $1; exit}' /src/.tmp-gio-settings-xwininfo.txt)
if [ -n "$window_id" ]; then
  import -display :99 -window "$window_id" /src/.tmp-gio-settings-shot.png
fi
