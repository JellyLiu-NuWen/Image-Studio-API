...
export function Section({
  ...
  return (
    <section className={`platform-card border border-black/[0.05] bg-white/70 shadow-[var(--shadow-card)] dark:border-white/[0.06] dark:bg-white/[0.03] ${isAndroidPhone ? "p-3" : "p-4"} ${isWindows ? "rounded-[12px]" : "rounded-[20px]"}`}>
...
export function SegItem({ active, onClick, children }: {
  ...
  return (
    <button
      ...
      } ${isWindows ? "rounded-[8px]" : "min-h-[34px] rounded-full"}`}
    >
      {children}
    </button>
  );
